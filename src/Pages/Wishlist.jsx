...
const Wishlist = () => {
  ...
  const handleRemove = async (id) => {
    const data = await myAxios.delete(
      `/delete-wishlist?email=${user?.email}&id=${id}`
    );
  ...
  if (list.length <= 0) {
    return (
      <Nodata
        text={"Your not any blog in you wishlist"}
        href={"/all-blogs"}
        buttonName={"Add from blogs"}
      />
    );
  }
  ...
  return (
    <div className="lg:container lg:mx-auto mx-4">
      <div className="my-16 text-center text-3xl font-semibold text-green-800 flex justify-center">
        <h1 className="border-b-2 pb-2 px-4 border-pink-600">
          Your Wishlisted blogs
        </h1>
      </div>
      {/* card */}
      <div className="grid grid-cols-1 lg:grid-cols-4 md:grid-cols-2 gap-5 ">
        {list?.map((data, idx) => (
          ...
          >
            ...
            <div className="p-5">
              <div className="flex">
                <h1 className="bg-cyan-500 px-2 rounded-lg">{data.category}</h1>
              </div>
              <a>
              ...
              <div className="flex justify-around">
                ...
                <a
                  ...
                  className="inline-flex cursor-pointer gap-1 items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                >
